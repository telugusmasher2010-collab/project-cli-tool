...
|---|---|
| **Tests** | Unit tests for every package, integration tests for CLI commands |
| **Error handling** | Every error has a message, a code, and a user-friendly output |
| **Logging** | Structured logging (info, warn, error, debug levels) |
| **CI/CD** | GitHub Actions runs tests on every push, releases on every tag |
| **Documentation** | README with install/usage/contributing, `--help` is actually useful |
| **Security** | No secrets in code, no command injection, input validation on all flags |
| **Semantic versioning** | Conventional commits + auto-changelog |
| **Cross-platform** | Windows, macOS, Linux — amd64 + arm64 — tested before every release |
| **Code reviews** | Every PR reviewed by at least 1 other team member |

---

## Phase 1 — You + Suhrit (Days 1-5)

Akshay not here yet. No waiting. Build the core.

| Area | What to build |
|---|---|
| **Go project skeleton** | Module init, cobra CLI scaffold, main.go, all package directories |
| **`init` command** | Interactive mode with prompts, template selection, path input |
| **`list` command** | Shows all available templates with descriptions |
| **Template engine** | `text/template` wrapper, variable substitution (`{{.ProjectName}}`, etc.) |
| **1st template (tauri-llm)** | Full Tauri v2 + Rust + React + local LLM sidecar — the most complex one |
| **2nd template (whatsapp-bot)** | Node.js + Baileys + SQLite + Fastify |
| **Colored output** | Spinners on long ops, green check ✓ on success, red ✗ on errors |
| **Error handling** | Custom error types with codes, user-friendly messages for every failure path |
| **Unit tests** | CLI parsing, template engine, errors — min 70% coverage on core packages |
| **Logging** | Structured levels: debug, info, warn, error |
| **Input validation** | Project names, paths, template names — no injection |
| **README** | Install, usage, contributing |
| **GitHub repo** | Created, collaborators added, branch protection rules |
| **`.gitignore`** | Go + generated files + secrets |
